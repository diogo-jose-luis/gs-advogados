...
const NAV = [
  ...
  { href: "/competencias", key: "nav.practices" },
  ...
  { href: "/podcast", key: "nav.podcast" },
  {
    ...
    children: [
      ...
      { href: "/informacoes-juridicas/orientacoes", key: "nav.legal.guides" },
      ...
      <nav className="bg-white/95 backdrop-blur border-b border-gray-100 shadow-header">
        <Container className="flex h-14 items-center justify-between gap-6">
          ...
          <Link href={withLocale("/")} className="shrink-0">
            <Image src="/logo-icon.png" alt="GS" width={28} height={28} priority />
          </Link>
          ...
          <ul className="hidden md:flex items-center gap-6 text-sm">
            {NAV.map(item => (
              <li key={item.key} className="relative group">
                <Link href={withLocale(item.href)} className="hover:text-gs-red transition">
                  {t(item.key)}
                </Link>

                {item.children && (
                  <div className="absolute left-0 mt-2 hidden group-hover:block bg-white border border-gray-100 shadow-lg rounded-xl p-2 w-64">
                    {item.children.map(c => (
                      <Link
                        key={c.key}
